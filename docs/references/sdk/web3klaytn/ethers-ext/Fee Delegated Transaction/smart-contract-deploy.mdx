# Smart Contract Deploy
**TxTypeSmartContractDeploy** deploys a smart contract to the given address. The following changes will be made by this transaction type.

<CH.Spotlight>
<CH.Code>

```js TxTypeFeeDelegatedSmartContractDeploy.js
const { Wallet, TxType } = require("@klaytn/ethers-ext");
const ethers = require("ethers");

const senderAddr = "0xa2a8854b1802d8cd5de631e690817c253d6a9153";
const senderPriv = "0x0e4ca6d38096ad99324de0dde108587e5d7c600165ae4cd6c2462c597458c2b8";
const feePayerAddr = "0xcb0eb737dfda52756495a5e08a9b37aab3b271da";
const feePayerPriv = "0x9435261ed483b6efa3886d6ad9f64c12078a0e28d8d80715c773e16fc000cff4";

async function main() {
  const provider = new ethers.providers.JsonRpcProvider("https://public-en-baobab.klaytn.net");
  const senderWallet = new Wallet(senderPriv, provider);
  const feePayerWallet = new Wallet(feePayerPriv, provider);

  let tx = {
    type: TxType.FeeDelegatedSmartContractDeployWithRatio,
    to:    "0x0000000000000000000000000000000000000000",
    value: 0,
    from: senderAddr,
    input: "",
    humanReadable: false,
    feeRatio: 30,
    codeFormat: 0x00,
  };

  tx = await senderWallet.populateTransaction(tx);
  console.log(tx);

  const senderTxHashRLP = await senderWallet.signTransaction(tx);
  console.log("senderTxHashRLP", senderTxHashRLP);

  const sentTx = await feePayerWallet.sendTransactionAsFeePayer(senderTxHashRLP);
  console.log("sentTx", sentTx);

  const rc = await sentTx.wait();
  console.log("receipt", rc);
}

main();
```




---
```zsh output
❯ node functionName.js
output
```
</CH.Code>






---

Import necessary modules for blockchain interaction.

```js TxTypeFeeDelegatedSmartContractDeploy.js focus=1:2

```
---
Define sender's and fee payer's addresses and private keys

```js TxTypeFeeDelegatedSmartContractDeploy.js focus=4:7

```
---

Create a connection to the Klaytn blockchain using a JSON-RPC provider


```js TxTypeFeeDelegatedSmartContractDeploy.js focus=10

```
---
Create wallets for the sender and fee payer using their private keys



```js TxTypeFeeDelegatedSmartContractDeploy.js focus=11:12

```
---
Define the transaction object for a fee-delegated smart contract deployment with a fee ratio of 30%



```js TxTypeFeeDelegatedSmartContractDeploy.js focus=14:23

```
---
Specify the transaction type as FeeDelegatedSmartContractDeployWithRatio


```js TxTypeFeeDelegatedSmartContractDeploy.js focus=15

```

---

Specify the contract bytecode

```js TxTypeFeeDelegatedSmartContractDeploy.js focus=19

```
---
Specify the [fee ratio](https://archive-docs.klaytn.foundation/content/klaytn/design/transactions/partial-fee-delegation).


```js TxTypeFeeDelegatedSmartContractDeploy.js focus=21

```
---
Populate the transaction with additional details (e.g., gas limit)


```js TxTypeFeeDelegatedSmartContractDeploy.js focus=25:26

```
---

Sign the transaction with the sender's private key

```js TxTypeFeeDelegatedSmartContractDeploy.js focus=28:29

```

---

Send the fee-delegated transaction using the fee payer's wallet
```js TxTypeFeeDelegatedSmartContractDeploy.js focus=31:32

```
---

Wait for the transaction  receipt

```js TxTypeFeeDelegatedSmartContractDeploy.js focus=34:35

```


</CH.Spotlight>
